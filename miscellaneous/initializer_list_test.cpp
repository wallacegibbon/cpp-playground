...
/// The programmer says {1, 2, 3, 4},
/// and the compiler creates an object of type initializer_list<int> out of it,
/// containing the same sequence of elements in it.
///
/// This contract is a requirement imposed by the language specification
...
int main(int argc, const char **argv) {
	...
	// mytest1{1, 2, 3, 4, 5, 6}; // error: this syntax is for Class constructors.

	Blah tmp1{10, 11, 12, 13}; /// same as : Blah tmp1({10, 11, 12, 13});
	tmp1.display();

	/// this is same as the previous initialization,
	/// it does NOT call the copy constructor
	Blah tmp2 = {10, 11, 12, 13};
	tmp2.display();

	/// the following example shows that the previous
	/// (Blah tmp2 = {...}) did NOT call the copy constructor

	// Blah tmp3 = tmp2; // error: use of deleted function ‘Blah::Blah(const Blah&)’

	/// just like those modern languages,
	/// you can create objects in the new way shown below
